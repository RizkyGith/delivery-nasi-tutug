...
class Pagination2
{
     ...
     */
    public function paginate($entries)
    {
        $this->app     = App::getInstance();
        $configCollection = $this->app->config('bono.providers')['Norm\\Provider\\NormProvider']['collections'];
        ...
        $paging = $paging. $this->nextPage($entries);
        $paging = $paging. $this->lastPage($entries);

        return  $this->wrapPagination($paging);
    }

    private function lastPage($entries)
    {
        $all     = count($entries);
    ...
    private function nextPage($entries) {
        $all     = count($entries);
        // $all     = $this->entries->count();
        $average = floor($all / $this->limit);
        if($this->current == $average) return '';
